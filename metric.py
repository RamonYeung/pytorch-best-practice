...
class CategoricalAccuracy(Metric):
    ...
    def compute(self):
        if self._num_examples == 0:
            raise ZeroDivisionError('CategoricalAccuracy must have at least one example before it can be computed')
...
class PRMetric(Metric):
    ...
    def __init__(self, num_class=2):
        ...
        self.confusion_matrix = np.zeros((self.num_class, self.num_class), dtype=np.float32)
